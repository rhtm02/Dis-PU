...
from losses.FeatureLoss import PerceptualLoss
...
from models.PuTransformer import PointUpsamplingTransformer,PUT_PT,NDPUT
from models.DisPU import DisPU
...
# model = PUT_PT(in_channels=16,out_channels=3).cuda()
#model = NDPUT(in_channels=3, out_channels_global=512, upscale_factor=4, std=0.05).cuda()
...
for e in range(EPOCH):
    ...
    train_cd_loss_coord = train_cd_loss_rev_coord = train_cd_loss = train_cd_loss_rev = train_uniform_loss = train_emd_loss = train_perceptual = 0
    SCORE = 1000
    ...
    for idx,(sparse_points,dense_points,_) in  enumerate(dataset):
        ...
        # dense_output = model(sparse_points)
        dense_output_coord,dense_output_offset = model(sparse_points)

        dense_output = dense_output_coord + dense_output_offset

        cd_loss, cd_loss_rev = chamfer_dist(dense_output, dense_points)
        cd_loss = torch.mean(cd_loss)
        cd_loss_rev = torch.mean(cd_loss_rev)

        cd_loss_coord, cd_loss_rev_coord = chamfer_dist(dense_output_coord, dense_points)
        cd_loss_coord = torch.mean(cd_loss_coord)
        cd_loss_rev_coord = torch.mean(cd_loss_rev_coord)

        #loss = CHAMFER * 0.5 * (cd_loss + cd_loss_rev) + UNIFORM * uniform_loss + EMD * emd_loss
        loss = ALPHA * (cd_loss + cd_loss_rev) + cd_loss_coord + cd_loss_rev_coord
        ...
        train_cd_loss_coord += cd_loss_coord.item()
        train_cd_loss_rev_coord += cd_loss_rev_coord.item()
        ...
        if (idx % 10) == 0:
            ...
            print(f'{e + 1}-EPOCH - ALPHA - {ALPHA} Q Chamfer Dist : {train_cd_loss / cnt:.6f} '
                  f'Q Chamfer Dist Rev : {train_cd_loss_rev / cnt:.6f} '
                  f'Q\' Chamfer Dist : {train_cd_loss_coord / cnt:.6f} '
                  f'Q\' Chamfer Dist Rev : {train_cd_loss_rev_coord / cnt:.6f}')
